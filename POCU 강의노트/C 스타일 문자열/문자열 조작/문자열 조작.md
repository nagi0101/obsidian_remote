...
- [[#문자열 비교|문자열 비교]]
	- [[#문자열 비교#프로토타입|프로토타입]]
	- [[#문자열 비교#비교 알고리즘|비교 알고리즘]]
	- [[#문자열 비교#표준함수|표준함수]]
- [[#문자열 복사|문자열 복사]]
	- [[#문자열 복사#표준 함수|표준 함수]]
	- [[#문자열 복사#비교적 안전한 문자열 복사: srtncpy()|비교적 안전한 문자열 복사: srtncpy()]]
	- [[#문자열 복사#strcpy() vs strncpy()|strcpy() vs strncpy()]]
- [[#문자열 합치기|문자열 합치기]]
	- [[#문자열 합치기#strcat()|strcat()]]
	- [[#문자열 합치기#보다 안전한 함수: strncat()|보다 안전한 함수: strncat()]]
- [[#문자열 찾기|문자열 찾기]]
	- [[#문자열 찾기#없는 문자열을 찾는 경우|없는 문자열을 찾는 경우]]
	- [[#문자열 찾기#프로토타입|프로토타입]]
	- [[#문자열 찾기#문자열 토큰화|문자열 토큰화]]
		- [[#문자열 토큰화#두 가지 깨닫는 점|두 가지 깨닫는 점]]
- [[#<string.h>의 다양한 함수들|<string.h>의 다양한 함수들]]
...
- 매개변수 str은 const char\*인데 반환값은 char\*임.
	...
	- 포인터를 받아와 쓰기를 수행할 경우 원본 문자열이 변경 가능한 문자열인 경우에만 strstr을 사용하는 것이 말이 됨. 좀 애매한 함수 정의.
		- 위의 예시 코드에서 str을 데이터섹션이 아니라 스택에 넣은 이유.
- 메모리 주소를 돌려주는 이유는 그게 빠르기 때문
	- substring을 반환하는 경우 새로운 문자열을 어디 저장하나?
		- 스택에 저장해봐야 받아올 수가 없으니 무의미
		- 힙에 저장하면 시스템 콜 일어나서 느림...
		- 그리고 더 이상 사용 안 하면 직접 반환해야 하는데 까먹고 안 할 수도 있음.
### 문자열 토큰화
- 위에서 말한대로 substring을 반환하는 것은 C에선 곤란
- 그러나 우아하고 고전적인 방법이 있음.
- 구분문자(delims): 콤마, 마침표, 스페이스
- 토큰화할 문자열(msg): Hi, there. Hello. Bye
```c
char* token = strtok(input, delims);
/* token: H "Hi\0 there. Hello. Bye\0" */
token = strtok(NULL, delims);
/* token: t "Hi\0 there\0 Hello. Bye\0" */
token = strtok(NULL, delims);
/* token: H "Hi\0 there\0 Hello\0 Bye\0" */
token = strtok(NULL, delims);
/* token: B "Hi\0 there\0 Hello\0 Bye\0" */
token = strtok(NULL, delims);
/* token: NULL "Hi\0 there\0 Hello\0 Bye\0" */
```
- 토큰화를 시작하려면 msg를 strtok() 에 넣음.
...
```
#### 두 가지 깨닫는 점
1. 토큰화하는 문자열은 const가 아니다! 원본이 바뀜.
2. 함수 매개변수로 NULL이 들어올 때 그 전에 받았던 msg를 사용하니 이건 어딘가에 저장되어 있어야 한다
	- 함수 내 정적 변수가 가장 적합할 것 같음.
- 이를 바탕으로 이 함수도 혼자 작성할 수 있어야 함.
...
- 꽤 많은 함수들이 문자열을 절대 변경하지 않는다.
	- 매개변수는 const char*
- 문자열을 변경해도 원본은 변경 안 하려 한다
	- 예외: strtok()
		- 원본을 지키려면 사본을 만든 뒤 strtok()를 호출
- 절대 새로운 문자열을 만들어주지 않는다.